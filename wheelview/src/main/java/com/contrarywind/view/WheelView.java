  ...
  public WheelView(Context context) {
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            dividerColor = a.getColor(R.styleable.pickerview_wheelview_dividerColor, 0xFFF4F4F4);
...
//                canvas.scale(1.0F, (float) Math.sin(radian));
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                  ...
                  paintOuterText.setTextSkewX((textXOffset == 0 ? 0 : (textXOffset > 0 ? 1 : -1)) * (angle > 0 ? -1 : 1) * DEFAULT_TEXT_TARGET_SKEWX
                      * offsetCoefficient);
                    // 控制透明度
